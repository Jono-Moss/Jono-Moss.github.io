...
The purpose of this guide, is to document my thought process and have a step by step guide to help me create Hugo themes for my future projects. Anybody who has had to maintain a "dynamic" web application that is accessed by the public becomes a security nightmare and at times a fulltime job. Frameworks such as wordpress has haunted me for years as a flame that attracts endless moths of malicious actors having a field day constantly probing and prodding your the site and server. I have always preferred to have "Static" based websites, and seeing the power of frameworks such as Hugo and Jekyll. It is about time I jump in and learn how to use them. I usually just develop my sites from scratch, but being able to have the a "blog" function where I can focus on just creating a well written .md file and have Hugo generate the pages for me will be a huge time saver. 

## Purpose of this guide.
I will only be going thought the basics of  learning how the layouts and basic content of a Hugo theme works. This guide will not be an ```html``` tutorial. I just wanted to see how to quickly get a basic theme started and will not be worrying about how the final design will look or even add and "responsive" development yet. I want to first understand the structure of a theme and how Hugo deals with the "content" of the site.

I will go into more depth in future guides and eventually end up using the theme I make as the main theme for this guide website. For now I will be using the beautiful - Stack https://github.com/CaiJimmy/hugo-theme-stack theme for this site but will move to my own theme once I am comfortable with my own theme.
...
2. In the ```layouts/``` folder. Are all the common html files that will be the main layout of all generated pages. Think of this as the main building blocks of your site.
...
4. The ```layouts/_default/``` these are the main "Wrapper / Containers" that will tie the desired components from the  ```layouts/partials/``` eg ```footer, header, menu``` to build the desired full page for the sites content to be displayed on.
...
6. The ```static/``` folder is where all the public files that will be copied to the public directory eg ```.htaccess, favicon.ico, robots.txt``` must be stored.

This is just a quick overview of the main folders that we will use / work in to create our own theme. For a full explanation of the entire folder and file structure, see https://gohugo.io/getting-started/directory-structure/
...
## Development
1. We will start by setting some global styles in the stylesheet.
...
5. add the logo.png file in the ```static``` folder.
...
category. To do so we can use the ```pre``` parameter in the ```[[menus.main]]``` sections in the themes ```hugu.toml``` file. 
...
14. in the themes ```hugu.toml``` file add the following parameter
```toml
...
Next i want to focus on the way Hugo deals with "assets" and "parameters" as I was not really successful in creating my own custom parameters that I wanted to use when creating this guide.   

## Security
1. Lastly, before I forget, I want to create an  ```.htaccess``` file to restrict access to subfolder on the site when it is live.
